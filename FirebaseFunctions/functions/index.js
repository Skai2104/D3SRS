...
exports.sendSOSToAuth = functions.firestore
									.document("Authorities/{user_id}/SOSNotification/{notification_id}")
									.onWrite((event, context) => {
										const user_id = context.params.user_id;
										const notification_id = context.params.notification_id;
										let from_user;
										...
										return admin.firestore().collection("Authorities").doc(user_id)
																.collection("SOSNotification").doc(notification_id)
																.get().then(queryResult => {
																	from_user = queryResult.data().from;
																	from_user_id = queryResult.data().fromId;
																	...
																	const from_data = admin.firestore().collection("Users").doc(from_user_id).get();
																	const to_data = admin.firestore().collection("Authorities").doc(user_id).get();
																...
																.then(result => {
																	...
																	const payload = {
																		notification: {
																			...
																			body: from_message,
																			icon: "default",
																			click_action: "com.skai2104.d3srsadmin.TARGETSOSNOTIFICATION"
																		},
																		data: {
																			message: from_message,
																			from_user: from_user,
